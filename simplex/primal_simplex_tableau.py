...
def is_tableau_optimal(tableau, basis_var, min):
    reduced_costs = tableau[0][1:-1]
    for idx, c in enumerate(reduced_costs):
        if idx in basis_var:  # fine
            continue
        if np.isclose(c, 0, rtol=0.000001):
            continue  # fix tolerance on reduced cost being almost pos/neg
        if (c < 0 and min is True) or (c > 0 and min is False):
            return False
    return True


def get_tableau_optimization(tableau, min):
    reduced_costs = tableau[0][1:-1]
    if min:
        cost_to_optimize = np.min(reduced_costs)
    else:
        cost_to_optimize = np.max(reduced_costs)

    j_star = (np.where(reduced_costs == cost_to_optimize))[0][0]

    A_bar_j_star = tableau[:, j_star + 1][1:]
    ...
    leaving_variable = optimal_theta[1]
    entering_variable = j_star  # nummer på variabel der indtræder
...
def iteration(tableau, min, basis_var):
    optimization = get_tableau_optimization(tableau, min)
...
def primal_simplex_tableau(A, b, c_T, basis_var, min=True):
    """
    Function to compute the optimum of a linear optimization problem using the primal simplex method
    Returns a optimal tableau.
    #### Assumptions
    * The provided LP-problem is the primal problem.
    * The provided basis is primal feasible.
    #### Parameters
    1. A: mxn list
    ...
    5. min: Boolean [optional, defaults to True]
            * A Boolean that represents if the given problem is a minimization or maximization problem.
    ...
    optimal_tableau = primal_simplex_tableau(A,b,c,basis_var)
    """
    np.set_printoptions(
    ...
    if not primal_usable(x_bar):
        return print(
            "There was an error with the provided values, which resulted in a basic feasible solution, that is not primal useable.\nThis means that the primal simplex method is not applicable."
        )
    ...
    while is_tableau_optimal(tableau, basis_var, min) is False:
        iteration_data = iteration(tableau, min, basis_var)
